...
function checkForWinDiagonal(winner) {
    if (fields[0] == fields[4] && fields[4] == fields[8] && fields[0]) {
        winner = fields[0];
        document.getElementById('line-7').style.transform = 'rotate(45deg) scaleX(1.33)';
    }

    if (fields[2] == fields[4] && fields[4] == fields[6] && fields[2]) {
        winner = fields[2];
        document.getElementById('line-8').style.transform = 'rotate(-45deg) scaleX(1.33)';
    }
    winOrTieRequest(winner);
}
...
function checkForTie() {
    if (fields[0] && fields[1] && fields[2] && fields[3] && fields[4] && fields[5] && fields[6] && fields[7] && fields[8]) {
        ActionsWhenGameIsOver();
    }
}


function winOrTieRequest(winner) {
    if (winner) {
        ActionsWhenGameIsOver();
    }
    else {
...
function ActionsWhenGameIsOver() {
    gameOver = true;
    setTimeout(function () {
...
function restart() {
    ...
    for (let i = 1; i < 9; i++) {
        document.getElementById('line-' + i).style.transform = 'scaleX(0.00)';
    }
